    ...
    template: function() {
        let elementTemplates = {
            ...
            table: `<div v-if="component == 'table'"
                         ...
                         v-bind:palette="content.chartOptions ? content.chartOptions.palette: null"
                         v-bind:order="content.chartOptions ? content.chartOptions.order: null"
                         v-bind:id="wrapperId"
                         >
...
let JsonDashboardifyUrlElement = Vue.component('json-dashboardify-url-element', {
    ...
    methods: {
        ...
        idifyData: function(prefix, data) {
            let i = 0, child = null, id = null, endpoint;
            endpoint = this.source.split(/\/+/g).pop();
            prefix = (endpoint + '-' + prefix).replace(/\./g,'-');
            data.id = prefix;
            if (data.children) {
